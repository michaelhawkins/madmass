  ...
  module Transaction
    module TxMonitor
      ...
      def tx_monitor &block
        ...
        begin
          if Madmass.config.tx_profiler
            tms = Benchmark.measure("TRANSACTION TIME") {
              Madmass.transaction do
                block.call
                #raise Java::OrgInfinispan::CacheException.new #REMOVE ME (USED ONLY FOR DEBUGGING)
              end
            }
            Madmass.logger.warn "#{Time.new.to_i * 1000} TRANSACTION TIME: real= #{tms.real} ; stime= #{tms.stime} ; utime= #{tms.utime} ; total= #{tms.total}"
          else
            Madmass.transaction do
              block.call
    	      #raise Java::OrgInfinispan::CacheException.new #REMOVE ME (USED ONLY FOR DEBUGGING)
          ...
          Madmass.logger.warn "EXCEPTION #{exc.message} - #{exc.class.name}"
          Madmass.logger.warn "\n********************************************************\n"
