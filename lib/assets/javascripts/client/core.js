...
Core.init = function(options) {
  ...
  $log = options.log ? Core.logger.log : function(){};
...
Core.Scheduler = new Class.Singleton({
  ...
  initialize: function(options){
    this.parent();
    $log('Core.Scheduler: initializing...');
    this.setOptions(options);
...
Core.Actor = new Class({
  ...
  assign: function(obj, options){
    switch($type(obj)){
      ...
      default:
        throw 'Invalid element assigned to the actor ' + this.objectId;
    }
    if(this.element){
      this.element.attr({
        id: this.objectId
...
Core.MainFrame = new Class.Singleton({
  ...
  initialize: function(){
    $log('Core.MainFrame: initializing...');
    this.parent();
    this.assign(document.body);
    $log('Core.MainFrame: OK');
  },
...
Core.Logger = new Class.Singleton({
  ...
  log: function(msg, options){
    options = options || {};
    options = $H(options).extend({time: true, level: 'info'});
    var out = '';
    if(options.time){
      var time = new Date;
