...
//    ]);
  ...
  registerCall: function(event, params, priority){
    if(!(event && params && params.agent && params.cmd)){
      $log("Core.Rpc: wrong remote call registration for event: " + event + " with params: " + JSON.stringify(params));
      return;
    }
    if(this.callMaps[event]){
      $log("Core.Rpc: trying to register an already registered remote call for event: " + event);
      return;
    }
    this.callMaps[event] = params;
    Core.dispatcher.listen({msg: event, listener: this, p : priority});
  },
  ...
  receive: function(msgId, data){
    ...
    if(callParams){
      ...
      $log("Core.Rpc: no registered call for event: " + msgId);
    }
    return true;
  },
  ...
  remoteCall: function(params){
    ...
    if(!(agentName && cmd)){
      $log("Core.Rpc: undefined agent call. Agent: " + agentName + ", call: " + JSON.stringify(params));
      return false;
    }
