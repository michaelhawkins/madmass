...
Core.Gui = new Class.Singleton({
  ...
  initialize: function(options){
    ...
    if(!this.namespace){
      $log('Core.Gui: invalid namespace for gui items: ' + this.namespace + '.Gui');
      return;
    }
  ...
  buildGui: function(items){
    ...
    items.each(function(item){
      ...
      try{
        ...
        $log('Core.Gui: error instantiating ' + itemClass);
      }
    }, this);
  }
...
Core.Gui.Item = new Class({
  ...
  fromTemplate: function(substitution){
    ...
    if(this.options.template == this.options.nullTemplate){
      ...
      if(element == null){
        throw "Game.Gui.Item: no template found for " + this.options.template;
      }
    }
    this.assign(element);
  },
